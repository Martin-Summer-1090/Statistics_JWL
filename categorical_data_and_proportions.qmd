...
library(waffle)
waffle(c("Death" = 260, "Survived" = 740), rows = 20, colors = c("#FD6F6F", "#93FB98"), use_glyph = "child", glyph_size = 2)
```
...
library(waffle)
waffle(c("Death" = 3, "Survived" = 997), rows = 20, colors = c("#FD6F6F", "#93FB98"), use_glyph = "child", glyph_size = 2)
```
...
with the most common operating systems such as Windows, OSX or Linux. We will use R in two
ways: First from the R command line and then also by working with Jupyter Notebooks. The
notebooks will not only allow to send command to R and execute these commands but in addition
it will allow to store command, comment them and make them available as files for later use.
...
## Example: Infant mortality rates in 2020 by global regions

As a first example of visualizing data in R I have prepared a data file from our initial
infant-mortality data which shows infant mortality rates for country groups with different
income-levels: Low and middle income, Low income, High income,  Upper middle income as well
as the infant mortality rate globally. I don't go here into the details how exactly these
data have been aggregated from individual country data. 

:::



A great visualization story is this for life expectancy:
https://www.youtube.com/watch?v=hVimVzgtD6w
Could we do something like this perhaps with infant mortality.?
