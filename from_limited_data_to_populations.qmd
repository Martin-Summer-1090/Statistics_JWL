...
Why do we look at data anyway? Looking at data, analyzing their
properties and learning about the information they contain can
be fun in its own. But most of the time when we look at data we want to
learn something about an entire population not only about the
usually much smaller group of people, objects, sites etc. which
comprise our data sets. In this section we learn about some
imporant considerations, notions and tools needed in such a process
of generalization. If done properly it can allow you to make a leap
from limited data to populations with a level of uncertainty you can at least quantify precisely. While we will learn about how to 
quantify uncertainty only later in this section we learn about
the most important considerations about inductively going from 
limited data to populations.

::: {.callout-note appearance="simple"}
## Overview

1. In this lecture you will learn about the process of **inductive inference**. Inductive
inference requires working from data to study sample and study population 
to target population.

2. At each stage **bias** can crop up. When you work with data and when you want to learn from data you need
to be aware of these potential **sources of bias**. We discuss and explain the most important
ones in this lecture.

3. The best way to proceed from a sample to the study population is if you have drawn a **random sample**. Using
R we will learn how random sampling works and why it helps to reduce bias.

4. We will discuss the idea of describing distributions by an approximation by the **normal curve**. This is an important idea in the entire field of statistics. The idea essentially says that a population can be thought of as a group of individuals but also as a probability distribution for a random observation drawn from that population

5. Population statistics often can be summarized using **parameters** that mirror the summary statistics of sample data. It often occurs that data do not arise as a sample from a literal population. We can always imagine data as drawn from a metaphorical population of events that could have occurred but didn’t.

6. In terms of R you will learn some powerful new concepts in this lecture, like **writing your own function**, performing **random draws from a population of data** as well as how to **determine the area under the normal curve** using R.
:::
...
**Sample**: A selected set of observations from a larger population of which we would ultimately like to learn its statistical properties.

**Study population**: The units that could have potentially be included in
the sample.

**Target population**: The population about which we ultimately hope to learn some statistical properties.

**Random sample**: A selection of observations from a study population where each unit has the same probability of ending up in the sample.

**Deductive reasoning**: Derives particular conclusions from general premises using the rules of logic.

**Inductive reasoning**: Starts from particular instances and treis to work out generalizations from there.

**Bias**: When our data systematically measure something different from what we actually want to measure.

**The normal curve**: A smoothed version of an idealized histogram which often occurs for large samples of empirical data. The normal curve is symmetric around the mean, the total area under the curve is 1 and the
curve is always above the horizontal axis. The area between minus and plus one standard deviation from the mean is 68%, the area within two standard deviations from the mean is 95% and the area within three standard deviations is 99.7% for the normal curve.

**Percentiles**: Percentiles are used to understand and interpret data. They indicate the values below which a certain percentage of the data in a data set is found.

**R-Functions**: R allows you to write functions of your own. A function needs a name, the name needs as assignment the term `function()`where the parantheses hold arguments of the function. The function body needs to be
within curly brackets {}. The last expression in the function body is the result a function computes when called.

:::


In the last chapter we analyzed data from the Demographic and Health Survey.^[To learn more about this survey, please visit the DHS website at https://dhsprogram.com/] We looked at data 
...
sweeping generalizations with some level of confidence. We will not learn here exactly how surveys, like the DHS are done but use the survey example to introduce you to some general ideas and principles of going from
...
![Random Sample](pictures/random_sample.png){#fig-randomsample}
...
```
Remember the human height data from exercise 16 in the last section on summarizing 
and communicating lots of data.
This was a data set with 25.000 observations and three variables, an index for each 
...
```
Each row of this table represents an individual. Each individual here is an adult person age 18 
for which a measure of height and weight has been taken. So if we sample from the rows of this
...
into a variable `idx`. Since we do not want row names we delete these. Deleting is achieved by
assigning `NULL` to the row names. `NULL` represents the null object in R. It is a reserved word like `TRUE`or `FALSE`and can not be used as name for an R object.  If you assign the null obejct to an R object this means that this object is deleted. Then we apply the subsetting 
...
course. But we all have an intuitive notion of probability from simple games of chance, such as from
...
@Spi2019, whom we have already cited before gives an example where this problem is particularly extreme. 
Often scientist who are ultimately interested in humans could onl study animals, as it is often the case
in pharmaceutical research. 

A bit less extreme but related is a second example @Spi2019 gives is the case of drugs tested say only on adults but it is then used also by children. These are problems which cannot entiely be solved by statistical methods but need caution and particular care and caution.
...
new field of data science got a big push from the attempt to get hold of 
data created through interactions on the
...
Still problems can occur. @Spi2019 mentions the problem that the police systematically misses cases which the
...
direct unit in which observations are measured - as for example cm for human height - but in units of standard deviations from the mean.
...
## Finding areas under the normal curve with R
...
Say we want to find the area under the normal curve between 0 an 1. Now one way to
find this area is to use you knowledge that 68 % of the area under the nomal curve are between
- 1 and 1. By the symmetry of the normal curve thus the area between 0 and 1 must be half of this area or 34 %.

When you use R to do this calculation the built in function is called `pnorm()`. This 
function gives you the area under the normal curve smaller or equal to a given 
value $x$. The default value for
the mean $\mu$ and the standard deviation $\sigma$ is $\mu = 0$ and $\sigma = 1$.
...
![Example 1: Area under the normal curve between 0 and 1](pictures/normal_curve_34.png)

Example 2:

Find the area between -2 and 1 under the normal curve with $\mu = 0$ and $\sigma = 1$.

This problem can be broken down by first computing the area under the normal curve for all
values lower than 1 and then the area under the curve for all values lower than - 2 and subtract
the latter from the former. In R:
...
![Example 1: Area under the normal curve between -2 and 1](pictures/normal_curve_84.png)

Example 3:

Find the area to the right of 1 under the normal curve. This can be done by finding the
area under the normal curve left from 1 first. Since the total area is 1, it must be the case
that 1 minus the value of the area left from 1 is the same as the area right of 1.
...
![Example 1: Area under the normal curve right of 1](pictures/normal_curve_16.png)

## The normal approximation for data

In certain cases the normal curve may be used to approximate data. We go back to our
example with the data on the height of adult humans.

Say we have the information that the height of adult humans were on average
`r round(mean(data$Height),2)` cm with a standard deviation of `r round(sd(data$Height), 2)` cm. 
Assuming the distribution of the data can be approximated by a normal curve. What is the
estimated percentage of humans with a height between 168 and 178 cm?
...
Let us check this approximation with the real data. We use logical subsetting to count the
share of height values between $l$ and $u$.
...
mean(idx)
```
You see that in this case the approximation works really well. So using the normal approximation
we can get a fairly precise generalization from study population to the target population, even 
though the target population is larger than our study population. What we have done is that 
we have replaced the histogram of the empirical data by a normal curve before computing areas.

Not all histograms follow a normal curve. But it is a remarkable fact, which we will explore in
more detail later in the course, that many histograms do. In this case the mean and the
standard deviation are good summary statistics.

## Percentiles

While the mean and the standard deviation are good statistics to summarize data
which follow a normal distribution, they are not so suited to summarize
other distributions.

Let us go back to one of these distributions we have encountered before: Primary energy
consumption per capita in the world in the year 2019.
...
The average primary energy consumption was `r format(round(mean(dat$Cons),2), scientific = F)` 
kwh per person per year and a standard deviation of `r format(round(sd(dat$Cons),2), scientific = F)` kwh per person per year. 

If we would just blindly apply a normal approximation using these statistics we would conclude that
21.15 % of countries in the world had a negative primary energy consumption. This does not make
sense. 

The reason for this nonsensical result is that the distribution depicted by the histogram
does not follow a normal distribution. To summarize such distributions, statisticians often
use so called **percentiles**. 
...
Let us first explain what you see in the code snippet. R has a built in functions to
compute percentiles. This function is called `quantile()`and takes the data as the first
argumente and then a vector of percentiles in decimal form. For example `quantile(dat$Cons, 0.01)`
would compute the 1 % percentile. 

What does this mean? The first percentile in our primary energy consumption distribution is
249.66. This means that in about 1 % of the countries of the world the primary energy consumption 
per person per year is 250 kwh or less. 99 % of the countries had a primary energy consumption 
above that level. The 50 % percentile (0.5) has a special name. It is called the
**median**. The median is often used as a measure of the center of the distribution for
cases where the data are distributed in a non-symmetric way. Since the median is so important
in practical work, R has a special function for the median which is called `median()`. If you take
```{r}
#| code-fold: false
median(dat$Cons)
```
...
difference between the 75 % percentile (0.75) and the 25 % percentile (0.25). This is
sometimes used as a measure of spread for non symmetric distributions with long tails, such 
as in this example. Also in this case R has a special function, calles `IQR()`. You may check
that it actually does what it shold do by computing:
...
Sometimes data are distributed like a normal curve and sometimes they are not. We will later
discuss a theoretical argument which helps to explain when precisely  histograms should be
...
we have explained in the previous discussion, in this example describe in words what precisely is 
...
1. On a certain exam, the average of the scores was 50 and the standard deviation was 10.
   a) Convert each of the following scores to standard units: 60, 45, 75.
   b) Find the scores which in standard units are: 0, 1.5, -2.8

2. Convert each entry on the following list to standard units (using the average and standard deviation of the list): 13, 9, 11, 7, 10. Find the average and the standard deviation of the converted list.
...
You can try the code we used in the text to select every 50iest row in the heights data 
yourself and apply it to another situation where you would for example select
every 100th observation in the dataframe.
...
and the standard deviation of the sample distribution of the Height variable. Compute - or look up in chapter 3 - the mean and the
...
Write a function which allows you to virtually throw a pair of dice and sum up the points shown after the throw.
Simulate and plot the result of your simulation. Are these dice fair of biased? Why?
...
Consider the values 175.9599 165.3240 169.0384 from a normal distribution with mean 172.7025 and
standard deviation 4.830264. Convert these values to standard units.
...
b) to the left of 0.80
c) between -0.3 and 0.9
d) to the left of -0.4
e) between 0.4 and 1.3
f) outside -1.5 to 1.5

2. Fill in the blanks:

a) The area between +/-    under the normal curve equals 68%
b) The area between +/-    under the normal curve equals 75 %.
...
Take the human height data from the JWL package `socr_height_weight` and compute
the 1%, 10%,  25%,  50%,  75%, 90%, 99% quantiles. Compare the median and the mean
and compare the standard deviation and the interquartile range. What do you observe?
Explain.
...
## Exercise 8: Practicing the tapply function

In the section about analyzing and communicating lots of data
we had used the `dhs` data `children_nutrtition_data` from the 
`JWL` package to compute the percentage of stunted children by
region. Try to compute the answer by using the `tapply()` function
instead of taking the approach we took before using subsetting rules.
Hint: There is a complication because we need to take into account the
weighting variable and not only the pure count of cases. When you
do the computation of proportions you need to create a new variable and
apply `tapply()`twice.
:::
