...
### Example 1: What are the major sources of electricity production in Kenya?
...
### Example 2: How many trees are there on the planet?
...
### Example 3: What has happened to extreme poverty on the globe in the last 20 years?
...
### Example 4: Does taking your time in college pay off?

A newspaper in Germany reported that the more semesters needed 
to complete and academic program at the university the greater 
the starting salary in the
first year of a job. The report was 
based on a study that used a random sample of 24 people 
who had recently
...
the job. The data are shown in the following plot.

```{r}
#| label: Years in college versus starting salary
...
the academic university program and the starting salary in the job. The
points you draw like this are "scattered" all over the place, but it 
seems that "by and large" there seems to be a trend, like this:
...
ggplot(dat, aes(x=Time, y=Salary)) +
     geom_point() +
...
really increasing with the semesters spent at the university. But is this pattern plausible? What do you think?

An independent researcher, who doubted the result, received 
the data from the 
newspaper and did a new analysis of the data by separating 
the data into three groups based on the major of each person.

```{r}
#| label: Years in college versus starting salary by major
...
computer for data exporation and visualisation. We assume no prior knowledge and start from skratch. We also do not strive for completeness. The idea is that you aqucire the practically most impirtant skills and get maturity to drill deeper for yourself after
...
will require lots of practice and participation from you to aqcuire this important skill.
...
only one component in a howl package of activities you will need to engage in when you work
...
of statistical analysis. Today these activities involve using a computer. In this course you will
...
use of the computer step by step. Using a computer requires a language in which we can tell the
...
Here we assume that students have a running R and R-Studio installation on their
...
Students get an indea about: uncertainty, empirical analysis and data display. There are many
