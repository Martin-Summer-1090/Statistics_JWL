...
This is an excellent opportunity to refresh some of the R skills you have required in this
course so far. The data that were used to produce this picture are in the JWL-package.

1. Load the JWL package
2. The dataset is called `enrol_attend_dat`. Use the R help function to study the
   data an the meaning of the variables as well as the original data source.
3. Can you write an R code reproducing the graphics? (Hint: Not everything you need to
   reproduce this plot was contained in the lectures. Try to use an internet search to find
   out for instance how to draw the blue line)
...
This is another excellent opportunity to refresh some of the R skills you have required in this
course so far. The data that were used to produce this picture are in the JWL-package.

The dataset is called `expenditure_outcome_dat`. Use R to compute the correlation coefficient
between public and private per capita expenditure of education and the average harmonized learning outcome. Explain what this number means.
...
1. Data from the polio vaccine studies discussed in the text suggest that in 1954, the
...
2. The polio vaccine studies were conducted only in certain experimental areas (school districts) selected by the Public Health Service in consultation with local officials. In these areas there were about 3 million children in grades 1,2, or 3; and there were about 11 million children in those grades in the United States. In the experimental areas, the incindence of
...
When you go back to the introductory chapter to this course, can you find the
example which showed Simpson's Paradox in a different context ? Which example was that?
Explain.
...
* Direct evidence
    ...
    + There is an appropriate temporaö and/or spatial proximity, in that cause precedes effect and effect occurs after a plausible interval and/or cause occurs at the same site as the
    effect.
...
* Mechanistic evidence
    + There is a plausible mechanism of action which could be biological, chemical or mechanical with external evidence fro a causal chain.
* Parallel evidence
