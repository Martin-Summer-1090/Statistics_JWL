...
for your statistical estimates as well as to predictions by using
...
Sample statistics computed using bootstrapping resamples are
...
contain 
elements of different types like − numbers, strings, vectors and another 
...
This can in geeneral best be done using the `vector()` function of R. It has
...
result the structure of a dataframe. Thus our output is a dataframe
...
give it a try. We first take 100 bootstrap resamples from our data and
compute the weightes means or prpoprtions for all four regions:
...
that the standard deviation is worst for region 3, where the mean can vary
...
fit is pretty good. This is no accident, as we will learn in the coming units.
...
Let us first produce a graph based on 20 resamples, which will lead to 20 
regression lines:
