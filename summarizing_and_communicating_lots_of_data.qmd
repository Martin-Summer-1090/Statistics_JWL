...
summarized, to understand their main characteristics. In this unit you will learn the
...
these summaries and what needs to be carefully considered. Clearly, when we need to deal
...
## Understanding variation in a single variable
...
annual flow is recorder are 100 Millions of cubic meters, i.e. $10^8 m^3$. 
...
fixed rule as to how to choose the size of these ranges. These ranges should neither be to fine, nor too
coarse. While there are a list of mechanical rules, which you can for example find on Wikipedia^[See https://en.wikipedia.org/wiki/Histogram#Number_of_bins_and_width], it is usually best to use your domain
...
when a flow value would for instance be measures as exactly 500, in which bin should it be counted:
400-500 or 500-600? You the constructor of the histogram has to take this decision. In practice you
...
a bar, which has the width of the bin and the height of the frequency. Do this for all the bins
...
bins, in our example. While loosing some local information on individua data points the global information
conveyed by the summary gives us a pretty good idea of the overall pattern of variation on the Nile river
flow data. We can see for instance, that the most frequent flow is between 800 and 900 and that the
...
Insert hand drawn histogram here
