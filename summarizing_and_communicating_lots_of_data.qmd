...
We see that `height_dat` is a data frame with three variables Index, Height and Weight. 
...
The variable `Height` in the dataframe `height_dat` is measured in inches. 1 inch is 2.54 cm.
...
height_dat$Height <- height_dat$Height*2.54
```
...
Use R to plot a histogram of the Height variabe in the height_dat dataframe.
...
```{r}
hist(height_dat$Height, xlab = "Height in kg", main = "Height of 18 year old humans")
```
...
Use R to compute the average or mean of the Height variable in the height_dat dataframe.

:::

```{r}
mean(height_dat$Height)
```
...
Compute the percentage of Heights in `height_dat` which are larger and the
percentage of Heights that are smaller than the mean of 172.7025.

:::

```{r}
mean(height_dat$Height > 172.7025)
mean(height_dat$Height < 172.7025)
```
